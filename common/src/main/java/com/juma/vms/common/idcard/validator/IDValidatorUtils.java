...
public class IDValidatorUtils {
	...
	public static void parseCode(IDCodeInfo code) {
		...
		String birthCode = (code.getType() == 18 ? body.substring(6, 14) : body
				.substring(6, 12));
		int order = Integer.parseInt(body.substring(body.length() - 3));
		String birth = "";
		if (code.getType() == 18) {
		...
		String addr = IDValidatorUtils.getAddrInfo(addrCode);
	...
	public static String getGenderByIdCardNo(String idCardNo){
		...
		if (idCardNo.length() == 15){
			if (Integer.parseInt(idCardNo.substring(14, 15)) % 2 == 0) {
				return "女";
			} else {
				return "男";
			}
